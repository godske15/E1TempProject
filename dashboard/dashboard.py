    ...
    if not numeric_df.empty:
        ...
        if len(latest_temps) >= 2:
            ...
            if not temp_in_range and highest_temp < 59:
                st.warning(f"⚠️ Warning: No temperature in optimal 59-60°C range. Highest: {highest_topic}: {highest_temp:.1f}°C")
        
        # Display alarm if triggered
        if alarm_triggered:
            st.error(alarm_message)
            st.markdown("""
            <style>
            .alarm-box {
            ...
            <div class="alarm-box">
                <h3 style="color: #d32f2f; margin: 0;">🔥 CRITICAL TEMPERATURE ALERT 🔥</h3>
                <p style="margin: 5px 0; font-size: 16px;"><strong>Current temperatures:</strong></p>
            """, unsafe_allow_html=True)
            
            for _, row in latest_temps.iterrows():
                color = "#d32f2f" if row['numeric_message'] > 55 else "#2e7d32"
                st.markdown(f"<p style='margin: 2px 0; color: {color}; font-size: 14px;'><strong>{row['topic']}: {row['numeric_message']:.1f}°C</strong></p>", unsafe_allow_html=True)
            
            st.markdown("</div>", unsafe_allow_html=True)
        else:
            # Show current status when no alarm
            if len(latest_temps) >= 1:
                st.success("✅ Temperature levels are within acceptable ranges")
                st.info(f"Current readings: " + " | ".join([f"{row['topic']}: {row['numeric_message']:.1f}°C" for _, row in latest_temps.iterrows()]))
